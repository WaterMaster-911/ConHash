...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,hash_length = 16):
...
#         self.hash_layer_conv = nn.Linear(int(256 * channel_ratio), int((self.hash_length)/2))        
        ...
        self.trans_cls_head = nn.Linear(int((self.hash_length)/2), num_classes) if num_classes > 0 else nn.Identity()
        
        self.pooling = nn.AdaptiveAvgPool2d(1)
        self.conv_cls_head = nn.Linear(int((self.hash_length)/2), num_classes)
    ...
    def forward(self, x):
        ...
        for i in range(2, self.fin_stage):
            x, x_t = eval('self.conv_trans_' + str(i))(x, x_t)  # 特征图转换部分

        # conv classification
        x_p = self.pooling(x).flatten(1)
        # hash_head
        hash_bit_conv = self.hash_layer_conv(x_p) 
        conv_cls = self.conv_cls_head(hash_bit_conv)
        # trans classification
        x_t = self.trans_norm(x_t)
        # hash_head
        hash_bit_trans = self.hash_layer_trans(x_t[:,0])   
        tran_cls = self.trans_cls_head(hash_bit_trans)     
        # hash_bit = torch.cat([hash_bit_one, hash_bit_two], dim=1)
        return [conv_cls, tran_cls] , [hash_bit_conv,hash_bit_trans] 
