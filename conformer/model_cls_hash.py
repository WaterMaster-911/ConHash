...
class Conformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # hash_head
        hash_bit_two = self.hash_layer_conv(x_p) 
        conv_cls = self.conv_cls_head(hash_bit_two)

        # trans classification
        x_t = self.trans_norm(x_t)
        # hash_head
        hash_bit_one = self.hash_layer_trans(x_t[:,0])   #  transformer bit
        tran_cls = self.trans_cls_head(hash_bit_one)

        hash_bit = torch.cat([hash_bit_one, hash_bit_two], dim=1)

        return [conv_cls, tran_cls] , hash_bit
