...
def config_dataset(config):
    ...
    if config["dataset"] == "voc2012":
        config["data_path"] = "./dataset/voc2012"
...
def compute_result(dataloader, net, device):
    ...
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        hash_list = net(img.to(device))[1]
        hash_bit = torch.cat([hash_list[0], hash_list[1]], dim=1)
        bs.append( hash_bit.data.cpu())
